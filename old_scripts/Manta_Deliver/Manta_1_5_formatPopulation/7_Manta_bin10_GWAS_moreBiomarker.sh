...
cd /home/zhiwei94/CNVscripts/Manta1_5Folder/FormatPopulationScripts/7_subScripts_MoreBiomarker
for i in {1..22}
do
...
source("/home/zhiwei94/CNVscripts/Manta1_5Folder/FormatPopulationScripts/7_WholeG_LinearRegression_moreBiomarker.R")
LR_function(pea_3, CNVbed, AgeSex, ${i})
EOM
done

for each in *.sh; do echo "sbatch ${each} > ${each%%.sh}.log"; done
# start with chr10 for testing:
#sbatch CNV_Pea3_Chr10.sh > CNV_Pea3_Chr10.log
