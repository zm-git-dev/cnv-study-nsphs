...
Pfilter <- 0.05/397/30598 #0.05/397/20598
histData_sort_filter <- histData_sort[histData_sort$P_value <= 0.05/397/30598,] #dim 23*9 0.05/397/30598; 25*9: 0.05/397/20598
significantBio <- data.frame()
library(stringr)
WholeGenome_filter <- T_Pdf[rownames(histData_sort_filter),] #size: 23biomarkers 108,745AllPos
...
sum(Manha23$P < 0.05/397/30598, na.rm = TRUE)
...
manhattan(Manha23_rmNA, genomewideline = FALSE, suggestiveline = FALSE, highlight = Manha23_rmNA$SNP[Manha23_rmNA$P < 0.05/397/30598] )
dev.off()
